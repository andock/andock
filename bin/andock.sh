...
REQUIREMENTS_ANDOCK_SERVER='0.0.5'
...
run_server_install ()
{
    ...
    ansible andock-docksal-server -e "ansible_sudo_pass=${andock_pw}" -e "ansible_ssh_user=andock" -i "${ANDOCK_INVENTORY}/${connection}"  -m raw -a "andock-server install" -vvv
    if [ "$tag" == "install" ]; then
